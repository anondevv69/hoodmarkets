...
import {IHoodMarketsLpLockerFeeConversion} from "./interfaces/IHoodMarketsLpLockerFeeConversion.sol";
...
contract HoodMarketsLpLockerFeeConversion is IHoodMarketsLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        factory = factory_;
        feeLocker = IHoodMarketsFeeLocker(feeLocker_);
    ...
    function placeLiquidity(
        ILiquid.LockerConfig memory lockerConfig,
        ILiquid.PoolConfig memory poolConfig,
        ...
        abi.decode(lockerConfig.lockerData, (IHoodMarketsLpLockerFeeConversion.LpFeeConversionInfo))
        .feePreference;
    ...
    function _mintLiquidity(
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            if (lockerConfig.tickLower[i] < poolConfig.tickIfToken0IsLiquid) {
        ...
        bool token0IsLiquid = token < poolConfig.pairedToken;
        ...
        int24 startingTick =
            token0IsLiquid ? poolConfig.tickIfToken0IsLiquid : -poolConfig.tickIfToken0IsLiquid;
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            uint256 amount0 = token0IsLiquid ? tokenAmount : 0;
            uint256 amount1 = token0IsLiquid ? 0 : tokenAmount;

            // determine tick bounds for this position
            int24 tickLower_ =
                token0IsLiquid ? lockerConfig.tickLower[i] : -lockerConfig.tickLower[i];
            int24 tickUpper_ =
                token0IsLiquid ? lockerConfig.tickUpper[i] : -lockerConfig.tickUpper[i];
            int24 tickLower = token0IsLiquid ? tickLower_ : tickUpper_;
            int24 tickUpper = token0IsLiquid ? tickUpper_ : tickLower_;
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!ILiquidHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the liquid, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the liquid token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.Liquid;
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only Liquid Factory can send NFTs here
        if (from != factory) {
            revert Unauthorized();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721Receiver).interfaceId
            || interfaceId == type(ILiquidLpLocker).interfaceId;
    }
}
