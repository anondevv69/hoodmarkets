...
contract HoodMarkets is OwnerAdmins, ReentrancyGuard, ILiquid {
    ...
    function setHook(address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the hook supports the ILiquidHook interface
        if (!ILiquidHook(hook).supportsInterface(type(ILiquidHook).interfaceId)) {
            revert InvalidHook();
        }
    ...
    function setLocker(address locker, address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the locker supports the ILiquidLpLocker interface
        if (!ILiquidLpLocker(locker).supportsInterface(type(ILiquidLpLocker).interfaceId)) {
    ...
    function setMevModule(address mevModule, bool enabled) external onlyOwnerOrAdmin {
        // check that the mev module supports the ILiquidMevModule interface
        if (!ILiquidMevModule(mevModule).supportsInterface(type(ILiquidMevModule).interfaceId)) {
    ...
    function setExtension(address extension, bool enabled) external onlyOwnerOrAdmin {
        // check that the extension contract supports the ILiquidExtension interface
        if (!ILiquidExtension(extension).supportsInterface(type(ILiquidExtension).interfaceId)) {
        ...
        returns (address tokenAddress)
    {
        ...
        tokenAddress = HoodMarketsDeployer.deployToken(tokenConfig, TOKEN_SUPPLY);
    }
        ...
        returns (address tokenAddress)
    {
        ...
        // deploy the token
        tokenAddress = HoodMarketsDeployer.deployToken(deploymentConfig.tokenConfig, TOKEN_SUPPLY);
        ...
        emit TokenCreated({
            ...
            poolId: poolKey.toId(),
            startingTick: deploymentConfig.poolConfig.tickIfToken0IsLiquid,
