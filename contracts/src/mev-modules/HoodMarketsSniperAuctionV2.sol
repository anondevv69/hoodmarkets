...
import {IHoodMarketsMevDescendingFees} from "./interfaces/IHoodMarketsMevDescendingFees.sol";
...
contract HoodMarketsSniperAuctionV2 is
    ReentrancyGuard,
    ILiquidSniperAuctionV0,
    IHoodMarketsMevDescendingFees,
    Ownable
{
    ...
    ILiquid public immutable liquidFactory;
    IHoodMarketsFeeLocker public immutable feeLocker;

    constructor(address owner_, address _liquidFactory, address _feeLocker, address _weth)
        Ownable(owner_)
    {
        liquidFactory = ILiquid(_liquidFactory);
        feeLocker = IHoodMarketsFeeLocker(_feeLocker);
        weth = _weth;
        ...
        view
    {
        ...
        // ensure that the associated hook is a HoodMarketsHookV2
        if (!IHoodMarketsHookV2(address(poolKey.hooks))
                .supportsInterface(type(IHoodMarketsHookV2).interfaceId)) {
            revert OnlyHoodMarketsHookV2();
        }

        // ensure the starting fee is not greater than the max mev LP fee
        if (feeConfigData.startingFee > IHoodMarketsHookV2(address(poolKey.hooks)).MAX_MEV_LP_FEE()) {
        ...
        if (
            feeConfigData.secondsToDecay
                > IHoodMarketsHookV2(address(poolKey.hooks)).MAX_MEV_MODULE_DELAY()
        ) {
        ...
        onlyHook(poolKey)
    {
        ...
        IHoodMarketsMevDescendingFees.FeeConfig memory feeConfigData =
            abi.decode(descendingFeeConfig, (IHoodMarketsMevDescendingFees.FeeConfig));
    ...
    function _sendPayment(PoolKey calldata poolKey, bool liquidIsToken0, uint256 paymentAmount)
        internal
    {
        if (paymentAmount == 0) {
            return;
        ...
        SafeERC20.safeTransfer(IERC20(weth), address(liquidFactory), factoryPayment);

        address liquid = liquidIsToken0
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // grab locker address from factory
        address lpLocker = liquidFactory.tokenDeploymentInfo(liquid).locker;

        // get reward info from the locker
        ILiquidLpLocker.TokenRewardInfo memory tokenRewardInfo =
            ILiquidLpLocker(lpLocker).tokenRewards(liquid);
    ...
    function _setLpFee(PoolKey calldata poolKey, uint24 lpFee) internal {
        ...
        IHoodMarketsHookV2(msg.sender).mevModuleSetFee(poolKey, lpFee);
    }
    ...
    function beforeSwap(
        ...
        _sendPayment(poolKey, liquidIsToken0, paymentAmount);
