...
contract HoodMarketsSniperUtilV2 is ReentrancyGuard {
    ...
    ILiquidSniperAuctionV0 immutable liquidSniperAuction;
    IUniversalRouter immutable universalRouter;
    IPermit2 immutable permit2;
    address immutable weth;

    constructor(
        address _liquidSniperAuction,
        address _universalRouter,
        address _permit2,
        address _weth
    ) {
        liquidSniperAuction = ILiquidSniperAuctionV0(_liquidSniperAuction);
        ...
        returns (uint256 txGasPrice)
    {
        ...
        if (desiredBidAmount % liquidSniperAuction.paymentPerGasUnit() != 0) {
            revert InvalidBidAmount();
        }

        // calculate the gas price for the bid amount
        txGasPrice = auctionGasPeg + (desiredBidAmount / liquidSniperAuction.paymentPerGasUnit());
    }
        ...
        nonReentrant
    {
        ...
        if (block.number != liquidSniperAuction.nextAuctionBlock(poolId)) {
            revert InvalidBlock();
        }

        // check that the gas price is high enough
        int256 paymentUnits = int256(tx.gasprice) - int256(liquidSniperAuction.gasPeg(poolId));
        if (paymentUnits < 0) {
            revert GasPriceTooLow();
        }

        // check proper msg.value was sent in
        uint256 expectedBidAmount = liquidSniperAuction.paymentPerGasUnit() * uint256(paymentUnits);
        ...
        IERC20(weth).approve(address(liquidSniperAuction), msg.value);
        ...
        if (IHoodMarketsHookV2(address(swapParams.poolKey.hooks))
        ...
        if (round + 1 != liquidSniperAuction.round(poolId)) {
